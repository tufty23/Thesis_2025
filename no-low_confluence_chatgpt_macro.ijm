...
// this is for auto Otsu thresholding with fill holes for no-low confluent images
...
for (i = 0; i < list.length; i++) {
    if (endsWith(list[i], ".jpg") || endsWith(list[i], ".png") || endsWith(list[i], ".tif")) {
        ...
        // setThreshold(0,200); - not doing manual thresholding as enhances background in no-low conf images
        run("Convert to Mask");
        run("Fill Holes");
        run("Open");
        run("Erode");
